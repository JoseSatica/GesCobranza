...
namespace CapaPresentacion{
    public partial class FrmAsignarMasivo : Form{
        ...
        private DataTable dtGestor;
        private DataTable dtGestorAntiguo;
        private DataTable dtGestorNuevo;
        ...
        private void CargarComboGestores(){
            dtGestor = NVerCartera.CargarDGVGestores();
            dtGestorAntiguo = dtGestor;
            dtGestorNuevo = dtGestor;
            foreach (DataRow fila in dtGestor.Rows) {
                CbxGestores.Items.Add(fila["GESTOR"].ToString().Trim());
                cbxGestorAntiguoCambiar.Items.Add(fila["GESTOR"].ToString().Trim());
                cbxGestorNuevoCambiar.Items.Add(fila["GESTOR"].ToString().Trim());
            }
        }
                ...
                if (cod_gestor != "" && prueba.RowCount > 1){
                    foreach (DataGridViewRow fila in prueba.Rows){
                        ...
                        if (existe == "1"){
                            ...
                            //decimal monto = Convert.ToDecimal(NAsignarMasivo.EnviarMonto(codigo).Rows[0]["suma"].ToString());
                            decimal monto = Convert.ToDecimal(NAsignarMasivo.EnviarMonto(codigo).Rows[0]["suma"].ToString());
                            if (monto <= 0 ){
                                continue;
                            }
                            //decimal suma = Convert.ToDecimal(monto);
                            //int id = Convert.ToInt32(NAsignarMasivo.EnviarCartera(id_gestor, codigo, 1, 1, monto).Rows[0]["id"].ToString());
                            //NCarteraDetalle.InsertarDetalleGestion(id, 1, "", "", "", "", "INICIO DE TRAMITE", monto, "", Environment.UserName.ToString(), Environment.MachineName.ToString(), Variables.cod_usuario);
                        }
                    }
                }
            }
        ...
        private void BtnArchivo_MouseHover(object sender, EventArgs e){
            TipAyuda.AutoPopDelay = 5000;TipAyuda.InitialDelay = 50;
        ...
        private void CbxGestores_SelectedValueChanged_1(object sender, EventArgs e){
            foreach (DataRow item in dtGestor.Rows){
                if (CbxGestores.Text == item["gestor"].ToString()){
        ...
        private void cbxGestorAntiguoCambiar_SelectedValueChanged_1(object sender, EventArgs e){
            foreach (DataRow item in dtGestorAntiguo.Rows){
                if (cbxGestorAntiguoCambiar.Text == item["gestor"].ToString()){
        ...
        private void cbxGestorNuevoCambiar_SelectedValueChanged_1(object sender, EventArgs e){
            foreach (DataRow item in dtGestorNuevo.Rows){
                if (cbxGestorNuevoCambiar.Text == item["gestor"].ToString()){
        ...
        private void CbxGestores_MouseClick(object sender, MouseEventArgs e){
            //CbxGestores.Items.Clear();
            //cargarComboGestores();
        }
        private void CbxGestores_DropDown(object sender, EventArgs e){
            //CbxGestores.Items.Clear();
            //CargarComboGestores();
        }
    }
